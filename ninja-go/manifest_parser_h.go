...
const (
	...
	KPhonyCycleActionError PhonyCycleAction = 1
)
...
func NewManifestParserOptions() *ManifestParserOptions {
	...
	ret.PhonyCycleAction = KPhonyCycleActionWarn
	return &ret
}
...
func (this *ManifestParser) ParseEdge(err *string) bool {
	...
	if len(edge.outputs_) == 0 {
		...
		this.state_.edges_ = this.state_.edges_[0 : len(this.state_.edges_)-1]
		//delete edge;
		return true
	}
	...
	if this.options_.PhonyCycleAction == KPhonyCycleActionWarn && edge.maybe_phonycycle_diagnostic() {
...
func (this *ManifestParser) Load(filename string, err *string, parent *Lexer) bool {
...
func (this *ManifestParser) ExpectToken(expected Token, err *string) bool {
	token := this.lexer_.ReadToken()
	if token != expected {

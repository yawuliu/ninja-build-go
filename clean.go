...
func (this* Cleaner)  IsVerbose() bool {
  return this.config_.verbosity != QUIET && (this.config_.verbosity == VERBOSE || this.config_.dry_run)
}

func (this* Cleaner) Remove(path string) {
  if !this.IsAlreadyRemoved(path) {
    this.removed_.insert(path);
